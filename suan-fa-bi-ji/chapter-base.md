...
## 4.5 二分
0. 解题套路：熟练写二分函数，`mid=(lo+hi)/2`的取值范围是`[lo,hi)`,注意此取值范围正确写上下界
1. `mid=(lo+hi)/2`，如果`lo+hi`会越界，则用`mid=(hi-lo)/2+lo`等价语句
2. 最常用的是`[lo,hi)`区间查找大于等于e的最小元素，或查找大于e的最小元素，两者的区别只是把`mid>=e`和mid>e
...
7. 快速幂：计算`a^b%m`，如果b达到了10^18，显然，递归相乘方法是不可取的。应采取快速幂方法。快速幂方法基于二分法，也称为二分幂。b为奇数时，`f(b)=a*f(b/2)^2`;b为偶数时，`f(b)=f(b/2)^2`.
